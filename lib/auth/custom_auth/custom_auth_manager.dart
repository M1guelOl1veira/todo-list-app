...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    toDoListAppAuthUserSubject.add(
      ToDoListAppAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<ToDoListAppAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  ToDoListAppAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = ToDoListAppAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    toDoListAppAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? UsuarioStruct.fromSerializableMap(
...
ToDoListAppAuthUser? currentUser;
